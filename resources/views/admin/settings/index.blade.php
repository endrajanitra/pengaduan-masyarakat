...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="desa_name" value="{{ old('desa_name', $settings->get('desa_name')?->value) }}" required
                        class="w-full px-3.5 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 {{ $errors->has('desa_name') ? 'border-red-300' : 'border-gray-300' }}">
                    @error('desa_name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Kepala Desa <span class="text-red-500">*</span></label>
                    <input type="text" name="kepala_desa" value="{{ old('kepala_desa', $settings->get('kepala_desa')?->value) }}" required
                        class="w-full px-3.5 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 {{ $errors->has('kepala_desa') ? 'border-red-300' : 'border-gray-300' }}">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="desa_kecamatan" value="{{ old('desa_kecamatan', $settings->get('desa_kecamatan')?->value) }}" required
                        class="w-full px-3.5 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 border-gray-300">
                </div>
                <div>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Alamat Kantor <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Email Kantor</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi Aplikasi</label>
